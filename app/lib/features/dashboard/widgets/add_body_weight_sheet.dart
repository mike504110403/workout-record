...
class _AddBodyWeightSheetState extends ConsumerState<AddBodyWeightSheet> {
  ...
  // 寫入路徑包 try/catch/finally——recordBodyWeight 拋錯時(例如 DB 寫入
  // 失敗、或 userId 解析不到拋出的 StateError)若沒有這層保護,_isSaving
  // 會永遠卡在 true,欄位與按鈕全部 disable、也沒有任何錯誤提示,使用者
  // 只能重開 app。失敗時秀 SnackBar 錯誤訊息;finally 裡確認 widget 還
  // mounted 才 setState 解除 loading(避免 pop 之後畫面已卸載還呼叫
